...
def read_snap(folder,FirstFile,LastFile,
    ...
    for iredshift in range(0,len(FullRedshiftList)):
        if RedshiftsToRead[iredshift]:              
            for ifile in range(FirstFile,LastFile+1):
                ...
                nGals += this_nGals
                f.close()
            print ("z=%s, nGals = %d" % (char_redshift,nGals))  
    ...
    for iredshift in range(0,len(FullRedshiftList)):
        if RedshiftsToRead[iredshift]:  
            #print("\nReading redshift: %0.2f" % FullRedshiftList[iredshift], "\n")
            print("\nReading redshift: %0.2f\n" % FullRedshiftList[iredshift])
            for ifile in range(FirstFile,LastFile+1):
                char_redshift="%0.2f" % FullRedshiftList[iredshift]
                filename = folder+'/'+'SA_z'+char_redshift+"_"+"%d"%(ifile)
                #print(filename)
                ...
                this_nGals = np.fromfile(f,np.int32,1)
                nGals += this_nGals
                print ("File %s  nGals = %d" % (ifile, this_nGals))  
...
import os
import psutil
def read_tree(folder,FirstFile,LastFile, props,template):    
    """ Reads L-Galaxy output files.
    Returns: (nTrees,nHalos,nTreeHalos,gals)
    Inputs: (folder,file_prefix,FirstFile,LastFile,props,template)
    props - list of properties to return
    ...
    for ifile in range(FirstFile,LastFile+1): 
        ...
        print ("File %d  nGals = %d" %(ifile,this_nGals))     
        
        #read galaxy catalogs
        full_this_gals = np.zeros(this_nGals,dtype=template) 
        ...
        for prop in template.names:
            if props[prop]:
                this_gals[prop] = np.copy(full_this_gals[prop])
        
        #copy into merged structure for all files
        gals[offset:offset+this_nGals] = np.copy(this_gals[:])    
        offset+=this_nGals
...
def read_tree_new(filename):  
    ...
    props = PropertiesToRead
    template = LGalaxiesStruct
    ...
    nskip=nbytes/4-3
    this_nGals = int(np.fromfile(f,np.int32,1))  
    #print ("File %s  nGals = %d\n" %(filename,this_nGals))           
    ib=np.fromfile(f,np.float32,int(nskip))       
       
    gals_all_props = np.fromfile(f,template,this_nGals) # all properties 
    gals = np.zeros(this_nGals,dtype=filter_dtype) # selected props
       
    for prop in template.names:
        if props[prop]:
            gals[prop] = gals_all_props[prop]
...
def merge_tree_files(Gals_list, folder,FirstFile,LastFile, props,template):   
    ...
    for ifile in range(FirstFile,LastFile+1):         
        ...
        print ("File %d  nGals = %d" %(ifile,this_nGals))           
        ib=np.fromfile(f,np.float32,int(nskip))            
        gals[offset:offset+this_nGals] = Gals_list[counter]
        counter += 1
        offset+=this_nGals
        #f.close()          
    #endfor    
...
def select_current_redshift(G_MR, ThisRedshiftList, ii, SnapshotList):
    ...
    if found_redshift==0:
        sys.exit("redshift: %0.2f not read" % ThisRedshiftList[ii]) 
...
def stellar_mass_with_err(G0_MR, Hubble_h, redshift):
    ...
    if(redshift>3.0):
        redshift =3.0
    error = 0.08*(1+redshift)      
    mass= np.log10(G0_MR['StellarMass']*1.e10/Hubble_h) + np.random.randn(len(G0_MR['StellarMass']))*error
            ...
            if label_type=='symbol':                  
                if(err_size>0.):
                    subplot.errorbar(x,y, yerr=err_size, fmt=sym, markersize=sym_size, 
                                     color=color, markeredgecolor=color,mfc=mfc, alpha=alpha)
                else:
...
def  write_to_file(x_axis, y_axis, file):
    fa = open(file, "w")
    fa.write("%d\n" % len(x_axis))
